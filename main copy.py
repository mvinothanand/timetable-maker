...
def find_slots(course, staff_class_merged_bmap, fn_first_hours, an_first_hours):
  ...
  rem_hours_temp = rem_hours
  
  
  it = 1
  while(rem_hours_temp > 0 and it <= 25):
    ...
    while(rem_hours_temp > 0 and iteration_count <= int(course["weekly_hours"]) ):
      ...
      print(f"Iteraton {it} {iteration_count}")      
      for day in week_days_shuffled:
        print(day, rem_hours_temp, end = " ")
        if rem_hours_temp <= 0:
          break
        ...
        matched_slot = get_slot(rem_hours_temp, max_block_size,curr_bmap, session_pref,skip_fn_first_hour , skip_an_first_hour)
        if (not matched_slot) and min_block_size < max_block_size:
          matched_slot = get_slot(rem_hours_temp, min_block_size,curr_bmap, session_pref,skip_fn_first_hour, skip_an_first_hour)

        if matched_slot:
          print(f"matched slot: {matched_slot}")
          #matched_slots[day].append(matched_slot)
          matched_slots[day] = matched_slots[day] + matched_slot
          rem_hours_temp = rem_hours_temp - len(matched_slot)
  ...
  if rem_hours_temp == 0:
    ...
    print(f"{Fore.RED}{Style.BRIGHT}Unable to find slots for {rem_hours_temp}")
